...
class JalaliCalendar : Calendar {
    ...
    constructor(
        ...
        set(
            yearMonthDate.year, yearMonthDate.month,
            yearMonthDate.date
        )
        complete()
    }
    ...
    internal constructor(
        year: Int, month: Int, dayOfMonth: Int, hourOfDay: Int,
        minute: Int, second: Int, millis: Int
    ) : super() {
        this[YEAR] = year
        this[MONTH] = month
        ...
        cal = GregorianCalendar(
            yearMonthDate.year,
            yearMonthDate.month, yearMonthDate.date, hourOfDay,
            minute, second
        )
    ...
    override fun computeFields() {
        ...
        if (!areFieldsSet) {
            ...
            // Week_Of_Month
            super.set(
                WEEK_OF_MONTH, weekOfYear(fields[6], fields[1])
    ...
    override fun add(field: Int, amount: Int) {
        ...
        if (field == MONTH) {
            ...
            val gc: Calendar = GregorianCalendar(
                yearMonthDate.year,
                yearMonthDate.month, yearMonthDate.date,
    ...
    override fun roll(field: Int, amount: Int) {
        ...
        when (field) {
            ...
            DAY_OF_YEAR -> {
                ...
                super.set(
                    DAY_OF_MONTH, jalaliDaysInMonth[internalGet(MONTH)]
                            - (temp - dayOfYear)
                )
            }
    ...
    companion object {
        ...
        val MIN_VALUES = intArrayOf(
            BCE, 1, FARVARDIN, 1, 0, 1, 1, SATURDAY,
            1, AM, 0, 0, 0, 0, 0, -13 * ONE_HOUR, 0
        )
        val LEAST_MAX_VALUES = intArrayOf(
            CE, 292269054, ESFAND, 52, 4, 28,
            365, FRIDAY, 4, PM, 11, 23, 59, 59, 999, 14 * ONE_HOUR,
            20 * ONE_MINUTE
        )
        val MAX_VALUES = intArrayOf(
            CE, 292278994, ESFAND, 53, 6, 31, 366,
            FRIDAY, 6, PM, 11, 23, 59, 59, 999, 14 * ONE_HOUR, 2 * ONE_HOUR
        )

        fun getGregorianDate(jalaliDate: String): Date {
            val arr = jalaliDate.split("/").toTypedArray()
            val jalali = YearMonthDate(
                arr[0].toInt(),
                arr[1].toInt() - 1, arr[2].toInt()
            )
            val gDate = jalaliToGregorian(jalali)
            val cal = getInstance()
            cal.clear()
            cal[gDate.year, gDate.month] = gDate.date
            return cal.time
        }

        fun getJalaliDate(gDate: Date?): String {
            val gCal = getInstance()
            gCal.clear()
            gCal.time = gDate
            val dt = gCal.time
            val myYear = gCal[YEAR]
            val myMonth = gCal[MONTH]
            val myDay = gCal[DAY_OF_MONTH]
            val gMyDate = YearMonthDate(myYear, myMonth, myDay)
            val jDate = gregorianToJalali(gMyDate)
            //        String jalali = jDate.year + "/" + (jDate.month + 1) + "/" + jDate.date;
            return jDate.year.toString() + "/" + String.format(
                "%02d",
                jDate.month + 1
            ) + "/" + String.format("%02d", jDate.date)
        }
        ...
        fun gregorianToJalali(gregorian: YearMonthDate): YearMonthDate {
            ...
            gregorianDayNo = (365 * gregorian.year
                    + Math.floor(((gregorian.year + 3) / 4).toDouble()).toInt()
        ...
        fun jalaliToGregorian(jalali: YearMonthDate): YearMonthDate {
            ...
            jalaliDayNo = 365 * jalali.year + ((jalali.year / 33)
                    * 8) + Math.floor(((jalali.year % 33 + 3) / 4).toDouble()).toInt()
            i = 0
            ...
            gregorianYear = 1600 + 400 * Math.floor((gregorianDayNo / 146097).toDouble()).toInt() /*
         * 146097
         * =
         * 365
         * *
         ...
         */
            ...
            if (gregorianDayNo >= 36525) /* 36525 = 365*100 + 100/4 */ {
                gregorianDayNo--
                gregorianYear += 100 * Math.floor((gregorianDayNo / 36524).toDouble()).toInt() /*
             * 36524
             * =
             * 365
             * *
            ...
            gregorianYear += 4 * Math.floor((gregorianDayNo / 1461).toDouble()).toInt() /*
         * 1461 =
         * 365*4 +
         * 4/4
         */
            ...
            i = 0
            while (gregorianDayNo >= gregorianDaysInMonth[i]
                + if (i == 1 && leap == 1) i else 0
            ) {
                gregorianDayNo -= (gregorianDaysInMonth[i]
                        + if (i == 1 && leap == 1) i else 0)
                i++
